...
class Throw:
    ...
    def __init__(
            self,
            input_score: str,
            input_methode: InputMethod = InputMethod.THREEDARTS) -> None:
        ...
        if not self.input_score.isdecimal():
            if self.input_methode == InputMethod.ROUND:
                ...
                if not self.input_score.startswith(
                        "d") and not self.input_score.startswith("t"):
                    raise ValueError(
                        f"Prefix {self.input_score[:1]} does not exist")
        ...
        if stripped_input_score.isdecimal():
            ...
            if self.input_methode == InputMethod.ROUND:
                if int_score in IMPOSSIBLE_SCORES or int_score > 180:
                    raise ValueError(
                        f"Input {self.input_score} is impossible to score")
            elif self.input_methode == InputMethod.THREEDARTS:
                if int_score not in SEGMENTS:
                    raise ValueError(
                        f"Input {self.input_score} is not a segment")
                if prefix == "t" and int_score == 25:
                    raise ValueError(
                        f"Input {self.input_score} is not a segment")
        else:  # fail if rest after prefix is not decimal
            raise ValueError(
                f"Input {self.input_score} does not match pattern")
